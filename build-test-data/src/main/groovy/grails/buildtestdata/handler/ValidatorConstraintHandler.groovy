...
class ValidatorConstraintHandler implements ConstraintHandler {
    ...
    void handle(GormEntity domain, String propertyName, Constraint appliedConstraint, ConstrainedProperty constrainedProperty, CircularCheckList circularCheckList) {
        ...
        if (!constrainedProperty?.validate(domain, domain.metaClass.getProperty(domain, propertyName), new MockErrors(this))) {
            throw new ConstraintHandlerException("Validator constraint not supported, attempted value (${domain.metaClass.getProperty(domain, propertyName)}) does not pass validation: property $propertyName of ${domain.class.name}")
        }
    }
}
